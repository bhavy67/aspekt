...
export async function getWallpapers({
  limit = 30,
  offset = 0,
}: { limit?: number; offset?: number } = {}): Promise<Wallpaper[]> {
...
export async function getWallpapersByMood(
  moodSlug: string,
  { limit = 30, offset = 0 }: { limit?: number; offset?: number } = {},
): Promise<Wallpaper[]> {
  const { data: mood } = await supabase.from('moods').select('id').eq('slug', moodSlug).single();
  if (!mood) return [];
